...
const Alerts = () => {
  const [alerts, setAlerts] = useState([
    { id: '1', type: 'shortage', severity: 'critical', medicine: 'Insulin (Regular)', location: 'Delhi, Mumbai, Bangalore', description: 'Critical shortage reported across major cities. Multiple pharmacies out of stock.', timestamp: '2024-07-19T10:30:00Z', status: 'active', assignedTo: 'Dr. Sharma' },
    { id: '2', type: 'price', severity: 'high', medicine: 'Levothyroxine 50mcg', location: 'Chennai, Hyderabad', description: 'Price surge detected - 300% increase from normal pricing.', timestamp: '2024-07-19T09:15:00Z', status: 'investigating', assignedTo: 'Team Alpha' },
    { id: '3', type: 'quality', severity: 'high', medicine: 'Metformin 500mg', location: 'Kolkata', description: 'Quality concerns reported - suspected counterfeit medicines.', timestamp: '2024-07-19T08:45:00Z', status: 'investigating' },
    { id: '4', type: 'supply-chain', severity: 'medium', medicine: 'Amlodipine 5mg', location: 'Punjab, Haryana', description: 'Supply chain disruption affecting northern states.', timestamp: '2024-07-18T07:20:00Z', status: 'active' }
  ]);
  ...
  const getSeverityInfo = (severity) => {
    switch (severity) {
      case 'critical': return { color: 'bg-red-500/20 text-red-300 border-red-500/30', border: 'border-l-red-500' };
      case 'high': return { color: 'bg-orange-500/20 text-orange-300 border-orange-500/30', border: 'border-l-orange-500' };
      case 'medium': return { color: 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30', border: 'border-l-yellow-500' };
      case 'low': return { color: 'bg-blue-500/20 text-blue-300 border-blue-500/30', border: 'border-l-blue-500' };
      default: return { color: 'bg-gray-500/20 text-gray-300 border-gray-500/30', border: 'border-l-gray-500' };
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <StatCard title="Critical Alerts" value={alerts.filter(a => a.severity === 'critical').length} icon={AlertTriangle} color="red" />
          <StatCard title="Active" value={alerts.filter(a => a.status === 'active').length} icon={Bell} color="orange" />
          <StatCard title="Investigating" value={alerts.filter(a => a.status === 'investigating').length} icon={Clock} color="yellow" />
          <StatCard title="Resolved" value={alerts.filter(a => a.status === 'resolved').length} icon={TrendingUp} color="green" />
      </div>

      {/* Filters */}
      <div className="flex items-center space-x-4 bg-black/20 backdrop-blur-xl border border-white/10 p-4 rounded-xl">
        <div className="flex items-center space-x-2">
          <label className="text-sm font-medium text-gray-300">Status:</label>
          <select
            value={filterStatus}
            onChange={(e) => setFilterStatus(e.target.value)}
            className="bg-white/5 border border-white/20 rounded-md px-3 py-1.5 text-sm text-white focus:ring-2 focus:ring-purple-500"
          >
      ...
      <div className="space-y-4">
        {filteredAlerts.map((alert) => {
            ...
            return (
                <div
                    key={alert.id}
                    className={`bg-black/20 backdrop-blur-xl rounded-lg p-6 border-l-4 ${severityInfo.border}`}
                >
                        ...
                        <div className="flex items-start space-x-4">
                            ...
                            <div className="flex-1">
                                <div className="flex items-center space-x-2 mb-2">
                                    ...
                                    <span className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${severityInfo.color}`}>
                                        {alert.severity}
                                    </span>
                        ...
                        <div className="flex items-center space-x-2">
                            {alert.status === 'active' && (
                                <button
                                    ...
                                    className="px-3 py-1 bg-yellow-600/80 text-white text-sm rounded-md hover:bg-yellow-700/80"
                                >
                                    Investigate
                                </button>
                            )}
                            {alert.status === 'investigating' && (
                                <button
                                    ...
                                    className="px-3 py-1 bg-green-600/80 text-white text-sm rounded-md hover:bg-green-700/80"
                                >
                                    Resolve
                                </button>
                            )}
                            <button onClick={() => dismissAlert(alert.id)} className="p-1 text-gray-500 hover:text-white">
                                <X className="w-4 h-4" />
                            </button>
                        </div>
                    </div>
      ...
      {filteredAlerts.length === 0 && (
        <div className="text-center py-12 bg-black/20 backdrop-blur-xl border border-white/10 rounded-xl">
